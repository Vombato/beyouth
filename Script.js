...
// In questo c'è la possibilità che il numero di tavolo sia vuoto se l'ordine è da asporto
function checkValuesTypeCashier(customerName, customerTable) {
    // Con questa funzione evito continui richiami alla pagina index in caso di form non validi
    var boolNameIsCorrect = (isNaN(customerName) && customerName != "");
    var boolEmptyButChecked = (document.getElementById("takeawayT").checked && customerTable == "");
    var boolFullNotChecked = (document.getElementById("takeawayF").checked && customerTable != "" && !isNaN(customerTable));
    
    if(boolNameIsCorrect && (boolEmptyButChecked || boolFullNotChecked)) {
        return true;
    }

    else {
        if(!boolNameIsCorrect) {
            document.getElementById('errorName').innerHTML = "Errore campo nome.";
        }
        else {
            document.getElementById('errorName').innerHTML = "";
        ...
        if(!boolEmptyButChecked || !boolFullNotChecked) { // può essere vuoto solo se è ceccato Si da asporto
            document.getElementById('errorTable').innerHTML = "Errore campo tavolo.";
        }
        else {
            document.getElementById('errorTable').innerHTML = "";
...
function isChecked() {
    ...
    if(document.getElementById("takeawayT").checked) { // Checcato SI asporto
        document.getElementById('tableNum').value = ""; // DA RIVEDERE
